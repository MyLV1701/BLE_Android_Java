...
public class JacksonAnnotationIntrospector extends AnnotationIntrospector implements Serializable {
    ...
    protected TypeResolverBuilder<?> _findTypeResolver(MapperConfig<?> mapperConfig, Annotated annotated, JavaType javaType) {
        ...
        _constructStdTypeResolverBuilder.init(jsonTypeInfo.use(), typeIdResolverInstance);
        ...
        _constructStdTypeResolverBuilder.typeProperty(jsonTypeInfo.property());
        Class<?> defaultImpl = jsonTypeInfo.defaultImpl();
        if (defaultImpl != JsonTypeInfo.None.class && !defaultImpl.isAnnotation()) {
            _constructStdTypeResolverBuilder.defaultImpl(defaultImpl);
        }
        _constructStdTypeResolverBuilder.typeIdVisibility(jsonTypeInfo.visible());
            ...
            if (javaType.hasRawClass(_classIfExplicit)) {
                ...
                try {
                    if (_classIfExplicit.isAssignableFrom(rawClass)) {
                        ...
                        throw new JsonMappingException(null, String.format("Cannot refine serialization type %s into %s; types not related", javaType, _classIfExplicit.getName()));
                    }
                } catch (IllegalArgumentException e2) {
                ...
                if (keyType.hasRawClass(_classIfExplicit2)) {
                    ...
                    try {
                        if (_classIfExplicit2.isAssignableFrom(rawClass2)) {
                            ...
                            throw new JsonMappingException(null, String.format("Cannot refine serialization key type %s into %s; types not related", keyType, _classIfExplicit2.getName()));
                        }
                    } catch (IllegalArgumentException e3) {
        ...
        if (contentType.hasRawClass(_classIfExplicit3)) {
            ...
            try {
                if (_classIfExplicit3.isAssignableFrom(rawClass3)) {
                    ...
                    throw new JsonMappingException(null, String.format("Cannot refine serialization content type %s into %s; types not related", contentType, _classIfExplicit3.getName()));
                }
            } catch (IllegalArgumentException e4) {
    ...
    protected BeanPropertyWriter _constructVirtualProperty(JsonAppend.Prop prop, MapperConfig<?> mapperConfig, AnnotatedClass annotatedClass) {
        ...
        virtualPropertyWriterInstance.withConfig(mapperConfig, annotatedClass, construct, constructType);
        throw null;
    }
}
