...
public class SleepUntil extends CommOperation {
    private byte[] bytes;
    ...
    public OperationResult execute(Context context, ConstantsManager constantsManager, Result result, LogSession logSession) {
        ...
        int sleep = getTarget().sleep(characteristic, this.bytes, true, logSession);
        if (sleep != 0) {
            logFail(result, logSession, "Characteristic value not equal to " + ParserUtils.bytesToHex(this.bytes, true));
            return toResult(sleep);
        }
        logSuccess(result, logSession, "Characteristic value equals " + ParserUtils.bytesToHex(this.bytes, true));
        return toResult(sleep);
    }
    ...
    public SleepUntil(@Attribute(name = "description") String str) {
        super(str);
    }
}
