...
public class SleepIf extends CharacteristicOperation {
    private byte[] bytes;
    ...
    protected void execute(IBluetoothLeConnection iBluetoothLeConnection) {
        ...
        if (characteristic != null) {
            iBluetoothLeConnection.sleep(characteristic, this.bytes, false, this.longValue, this.server);
        }
    }
    ...
    public Operation[] mirror(DatabaseHelper databaseHelper) {
        ...
        if (!TextUtils.isEmpty(this.value)) {
            return new Operation[]{new Sleep("NOTE: The other side is waiting " + str + "until the value of " + getName(databaseHelper) + " is not equal to 0x" + this.value, 0L)};
        }
        if (TextUtils.isEmpty(this.valueString)) {
            return new Operation[]{new Sleep("NOTE: The other side is waiting " + str + "until the value of " + getName(databaseHelper) + " is not empty", 0L)};
        }
        return new Operation[]{new Sleep("NOTE: The other side is waiting " + str + "until the value of " + getName(databaseHelper) + " is not equal to '" + this.valueString + "'", 0L)};
    }
    ...
    protected boolean validateResult(IBluetoothLeConnection iBluetoothLeConnection) {
        BluetoothGattCharacteristic characteristic = getCharacteristic(iBluetoothLeConnection);
        return (characteristic == null || Arrays.equals(characteristic.getValue(), this.bytes)) ? false : true;
    }

    public SleepIf(DatabaseHelper databaseHelper, IBluetoothLeBasicConnection iBluetoothLeBasicConnection, BluetoothGattCharacteristic bluetoothGattCharacteristic, byte[] bArr, long j, boolean z) {
