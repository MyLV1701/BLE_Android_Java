...
rem 3. nRF Toolbox (1.6.0+) or nRF Connect (previously nRF Master Control Panel) (1.8.0+) application installed on the Android device
...
rem Read the optional device Bluetooth LE address. If the address has not been provided user may select the device on the phone/tablet
...
if "%ADDRESS%"=="" (
 ...
 -e no.nordicsemi.android.dfu.extra.EXTRA_FILE_PATH "/sdcard/Nordic Semiconductor/Upload/%HEX_FILE%" %E_INIT_FILE% | find "Error" > nul 2>&1
	...
	call adb %S_DEVICE% shell am startservice -n no.nordicsemi.android.mcp/.dfu.DfuServiceaa -a no.nordicsemi.android.action.DFU_UPLOAD^
